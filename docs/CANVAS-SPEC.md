...
| Examples and templates | **Shipped** | Twenty-five finished tables in six categories (`tableExamples.ts`), in the panel's gallery and the dock's Table flyout, each drawn as itself by `tableToSvg`. Most compute — totals, variances, weighted scores, averages — and every status tint is a live colour rule; a test asserts none computes to an error. The gallery marks calculating tables and finds them by function name. Four board templates are built from the same specs. |
| Colour rules | **Shipped** | Conditional formatting per column: a COUNTIF-style criterion and a paint, read against the cell's value (formula results included), first match wins, following the column through structural edits. |
| Fit to content | **Shipped** | Measured with the renderer's own font (`tableFit.ts`, `tableMeasure.ts`). Typing widens a column in the same undo step, up to a cap, unless the table's `autoFit` is off; double-clicking an edge fits one column, the toolbar and panel fit all, CSV imports arrive fitted. Adding a column grows the table rather than squeezing the others. |
| Direct structure | **Shipped** | A `+` on every row and column boundary in lanes past the gutters, add strips on the right and bottom edges, drag a selected whole row or column to reorder it (styles, intact merges, sort and filter follow), and right-click menus on letters, numbers and cells. The last column's edge resizes the table. |
| Formulas | **Shipped** | `=`-prefixed cells, Excel grammar and precedence, ~35 functions including COUNTIF/SUMIF/AVERAGEIF, evaluated without `eval` — table text replicates to the room. References follow the gutter labels and are rewritten on insert, delete, move and commit-the-view; cycles show `#CYCLE!`; sort, filter and CSV read results. The editor suggests functions, shows the signature being typed, previews the result, outlines the cells a formula reads and writes references from clicks and drags. |
| **Not built** | **Absent** | Per-row heights, wrapped text (long text is cut with an ellipsis), whole-row colour rules, references between tables, relative references that adjust on paste or fill, and per-cell presence for two editors. |
