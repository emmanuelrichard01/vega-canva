...
export function terminateRun(
  ...
  /**
   * The direction the run leaves and arrives, from its own last segment.
   *
   * Briefly there was an `axis` option here that pointed a profiled line's caps
   * along its overall direction instead, because a wave arrived at 55° to its
   * own axis and the head looked cocked. That treated the symptom: the head
   * then pointed the right way and sat crooked against the stroke reaching it.
   * The wave and the zigzag now *arrive flat*, like the coil always did, so the
   * last segment is the right answer for every run and there is no special case
   * to get wrong. See `linePath.PROFILE_LEAD`.
   */
