...
import { DEFAULT_CONNECTOR_INK, DEFAULT_INK, DEFAULT_TYPOGRAPHY } from '../model/schema';
...
export function buildDiagram(
  ...
  for (const node of graph.nodes) {
    ...
    nodes.push({
      ...
      appearance: {
        // A `style` or `classDef` in the source wins over the defaults — the
        // colour in a flowchart is nearly always saying something.
        fill: [{ type: 'solid', color: node.style?.fill ?? '#FFFFFF' }],
        stroke: { color: node.style?.stroke ?? DEFAULT_INK, width: node.style?.strokeWidth ?? 2 },
  ...
  for (const edge of graph.edges) {
    ...
    nodes.push({
      ...
      height: 1,
      [DIAGRAM_TAG]: diagramId,
      from: { nodeId: idFor(edge.from), port: 'auto' },
      to: { nodeId: idFor(edge.to), port: 'auto' },
...
export function diagramToMermaid(
  ...
  const graph: MermaidGraph = {
    ...
    nodes: ordered.map((node) => ({
      ...
      // The visible label, falling back to the key so a blank box still
      // produces parseable source rather than `A[]`.
